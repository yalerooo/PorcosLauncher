  ...
  return new Promise((resolve, reject) => {
    ...
    win.webContents.session.once('will-download', (event, item) => {
      ...
      item.once('done', async (event, state) => {
        if (state === 'completed') {
          try {
            // Read file signature to determine file type
            ...
            // Handle different archive types based on signature
            if (['504B0304', '504B0506'].includes(signature)) {
              ...
              try {
                ...
                // Get all file names from the iterator
                const fileNames = [];
                for (const header of list.fileHeaders) {
                  fileNames.push(header.name);
                }
                
                // Extract files using the collected names
                const extracted = extractor.extract({ files: fileNames });
                
                // Write extracted files to destination directory
                for (const file of extracted.files) {
                  ...
                  // Write file (only if extraction exists)
                  if (file.extraction) {
                    fs.writeFileSync(filePath, Buffer.from(file.extraction));
                  }
                }
                
                // Delete the RAR file if not keeping archive
                if (!keepArchive) {
                  fs.unlinkSync(savePath);
                }
                resolve({ success: true, path: destinationDir });
